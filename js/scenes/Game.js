class Game extends Phaser.Scene {
  ...
  preload() {
    ...
    progressBox.fillStyle(0x888888, 0.8);
    var width = this.game.canvas.width;
    var height = this.game.canvas.height;
    progressBox.fillRect(width / 2 - 200, height / 2 - 20, 400, 25);
    ...
    this.load.scenePlugin({
      key: "rexuiplugin",
      url: "js/util/rexuiplugin.min.js",
      sceneKey: "rexUI",
    });
  }
  ...
  create() {
    ...
    this.bobNpc = new BobNpc({ scene: this, x: 430, y: 530, key: "npc1" });
    this.trisNpc = new TrisNpc({ scene: this, x: 848, y: 740, key: "npc0" });
    this.connectedPlayersText = this.add
      .text(10, 10, "Connected players: ", {
        font: "16px Arial",
        fill: "#000000",
  ...
  update() {
    ...
    if (this.mainPlayer != undefined) {
      ...
      this.client.move(
        this.mainPlayer.playerSprite.x,
        this.mainPlayer.playerSprite.y
      );
    }
  ...
  addPlayer(id, x, y, randomClass) {
    this.players[id] = new OtherPlayer({
      id: id,
      scene: this,
      x: x,
      y: y,
  ...
  processUpdate(enemies, players, projectilles, diff) {
    ...
    this.connectedPlayersText.setText([
      "Use joystick or press W, S, A or D to walk.",
      "Touch or click to use your skill.",
      "",
      "Connected players: " + players.length + ".",
      "Ping: " +
        (this.client.socket.ping != undefined
          ? this.client.socket.ping + " ms, "
          : "calculating, ") +
        " diff: " +
        diff +
        " ms, fps: " +
        Math.floor(this.game.loop.actualFps),
    ]);
  }
  processProjectilles(projectilles) {
    ...
    for (var i = 0; i < projectilles.length; i++) {
      ...
      if (shouldAdd) {
        tempProjectillesArray.push(
          this.physics.add.sprite(
            projectilles[i].x,
            projectilles[i].y,
            projectilles[i].class
          )
        );
        tempProjectillesArray[tempProjectillesArray.length - 1].id =
          projectilles[i].id;
        tempProjectillesArray[tempProjectillesArray.length - 1].class =
          projectilles[i].class;
        tempProjectillesArray[tempProjectillesArray.length - 1].rotation =
          projectilles[i].rotation;
        tempProjectillesArray[tempProjectillesArray.length - 1].speed =
          projectilles[i].speed;
        tempProjectillesArray[tempProjectillesArray.length - 1].isPlayerParent =
          projectilles[i].isPlayerParent;
      }
    }
  ...
  processUpdateItems(items) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (shouldAdd) {
        tempItemsArray.push(
          this.add.sprite(items[i].x, items[i].y, items[i].class).setDepth(2)
        );
        ...
        if (Utils.distance(player.playerSprite, playerData) < 300) {
          player.tween = this.tweens.add({
            ...
            onUpdate: function () {
              ...
              player.healthBar.x =
                player.playerSprite.x - player.playerSprite.displayWidth / 2;
